...
contract RentalAgreement is  ReentrancyGuard {
    ...
    function listEquipments(uint256 tokenId, uint256 rentAmount, uint256 securityDepositeAmount) external notRented(tokenId) {
    IERC721 nft = IERC721(nft_address);

    // Check if the caller is the owner of the NFT
    address actualOwner = nft.ownerOf(tokenId);
    require(msg.sender == actualOwner, "Only NFT owner can list");

    // Check if the contract is approved to manage the NFT
    require(
        nft.isApprovedForAll(actualOwner, address(this)) || 
